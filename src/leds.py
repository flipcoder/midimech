...
def led_grid_worker(command_queue, width=24, height=16):
    import xled
    class LEDGridDisplay:
        def __init__(self, width=24, height=16):
            ...
            try:
                ...
                print("No device found, running in simulation mode")
                self.ctr = None
            
            # Block configuration
            self.BLOCK_FLAGS = [set() for _ in range(6)]
            self.LBLOCK = [0] * 6
            self.PBLOCK = [0] * 6
        ...
        def _setup_block_config(self):
            ...
            self.BLOCK_FLAGS[3].add('v')
            self.BLOCK_FLAGS[4].add('v')
            self.BLOCK_FLAGS[5].add('v')
            
            # Logical to Physical block mapping
            self.LBLOCK[0], self.PBLOCK[3] = 3, 0
            self.LBLOCK[1], self.PBLOCK[4] = 4, 1
            self.LBLOCK[2], self.PBLOCK[5] = 5, 2
            self.LBLOCK[3], self.PBLOCK[2] = 2, 3
            self.LBLOCK[4], self.PBLOCK[1] = 1, 4
            self.LBLOCK[5], self.PBLOCK[0] = 0, 5

        def _setup_orientations(self):
            r = 0
            for lblock in range(6):
